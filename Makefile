...
all:  clean
	...
	wget --output-document="$(PWD)/build/build.deb" https://download3.operacdn.com/pub/opera-beta/78.0.4093.103/linux/opera-beta_78.0.4093.103_amd64.deb
	...
	echo '' >> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '' >> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo 'exec $${APPDIR}/opera/opera-beta "$${@}"' >> $(PWD)/build/Boilerplate.AppDir/AppRun
	...
	export ARCH=x86_64 && $(PWD)/bin/appimagetool.AppImage $(PWD)/build/Boilerplate.AppDir $(PWD)/Opera-Beta.AppImage
	chmod +x $(PWD)/Opera-Beta.AppImage
